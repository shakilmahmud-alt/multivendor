    ...
    <div className="bg-slate-50 min-h-[60vh] flex items-center justify-center py-20 px-4">
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-10 max-w-2xl w-full text-center">
        ...
        <p className="text-sm text-slate-600 mb-8 font-medium">
          Your payment has been successfully processed and your order - <strong className="text-brand-500">{String(id).substring(0, 6).toUpperCase()}</strong> has been placed.
        </p>

        <div className="flex flex-col items-center gap-4 max-w-xs mx-auto">
          <Link 
            to="/track-order" 
            className="w-full bg-brand-500 hover:bg-brand-600 text-white font-bold py-3 px-6 rounded-md transition text-sm shadow-md"
          >
            Track Order
          </Link>
          <Link 
            to="/" 
            className="text-slate-600 hover:text-brand-500 text-sm font-semibold transition flex items-center justify-center gap-1"
          >
            Continue Shopping
          </Link>
        </div>
