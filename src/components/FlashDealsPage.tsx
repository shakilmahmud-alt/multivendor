...
export default function FlashDealsPage({ allProducts, onAddToCart, onAddWishlist, onSelectProduct, onQuickView, wishlist }: FlashDealsPageProps) {
  ...
  // Fetch active flash deals products
  useEffect(() => {
  ...
  // Filter products belonging to active flash deal
  const flashProducts = useMemo(() => {
    return allProducts.filter(p => flashProductIds.includes(String(p.id)));
  }, [allProducts, flashProductIds]);

  // Extract available brands for flash deals
  ...
  const filteredProducts = useMemo(() => {
    let result = flashProducts.filter(p => p.price >= minPrice && p.price <= maxPrice);
    ...
    return result;
  }, [flashProducts, minPrice, maxPrice, selectedBrands, sortOrder]);
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="bg-white p-4 rounded border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
              <div>
                <h1 className="text-xl font-bold text-slate-800">Flash Deals Products</h1>
            ...
            {/* Product Grid */}
            {loading ? (
              <div className="text-center py-12 text-slate-500 font-semibold">Loading Flash Deals...</div>
